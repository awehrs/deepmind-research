...
class Experiment(experiment.AbstractExperiment):
    ...
    def _forward_fn(
        ...
        decoder = perceiver.ClassificationDecoder(
            self.config.data.num_classes, **perceiver_kwargs["decoder"]
        )
    ...
    def _initialize_train(self):
        ...
        self._optimizer = utils.make_optimizer(self.config.optimizer, self._lr_schedule)
    ...
    def _loss_fn(
        ...
        logits, state = self.forward.apply(params, state, rng, inputs, is_training=True)
        ...
        if "mix_labels" in inputs:
            ...
            label = mix_ratio * label + (1.0 - mix_ratio) * mix_label

        # Apply label-smoothing to one-hot labels.
        label_smoothing = self.config.training.label_smoothing
        if not (label_smoothing >= 0.0 and label_smoothing < 1.0):
            raise ValueError(
    ...
    def _update_func(
        ...
        scaled_grads, (loss_scalars, state) = grad_loss_fn(params, state, inputs, rng)
    ...
    def _eval_batch(
        ...
        """Evaluates a batch."""
        logits, _ = self.forward.apply(params, state, rng, inputs, is_training=False)
        ...
        return {
            "eval_loss": loss,
            "eval_top_1_acc": top_1_acc,
            "eval_top_5_acc": top_5_acc,
        }
